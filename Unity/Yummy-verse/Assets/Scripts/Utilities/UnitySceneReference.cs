...
namespace Utilities {
	...
	///
	/// Using <see cref="ISerializationCallbackReceiver" /> was inspired by the <see
	/// cref="https://github.com/JohannesMP/unity-scene-reference">unity-scene-reference</see> implementation.
	/// </summary>
#if UNITY_EDITOR
	[InitializeOnLoad]
#endif
...
#pragma warning restore 0414
		...
		public override bool Equals(object obj) {
			if(obj is SceneReference other) { return Equals(other); }
		...
		public void OnBeforeSerialize() {
			...
			if(s_ReloadingAssemblies) return;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			if(GUI.Button(buildSettingsPos, settingsContent, EditorStyles.miniButtonRight) && sceneAssetProperty.objectReferenceValue) {
				if(indexInSettings != -1) {
					...
					var newScenes = new EditorBuildSettingsScene[] { new EditorBuildSettingsScene(
